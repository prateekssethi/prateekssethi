## 👋 Hi, I'm Prateek Sethi

📊 **Data Visualizer & BI Developer**  
🎯 Passionate about transforming data into actionable insights

I design & develop interactive and user-friendly dashboards, with a focus on performance, clarity, and data storytelling. I also bring UI/UX design skills to create dashboards that are not just functional but intuitive.

## 💼 Core Skills
- **Microsoft Fabric | Copilot | Power BI | DAX | Power Query | SQL | Excel**
- **Data Modeling | Dashboard Design | KPIs & Smart Narratives**
- **MicroStrategy | Tableau | SSMS | MySQL**
- **UI/UX Design (Figma, Adobe XD)**

## 📬 Let's Connect!
- 🔗 **LinkedIn**(https://www.linkedin.com/in/prateeksethi24/) 
- 🔗 **website**(https://prateeksethi.netlify.app/)  
- 🔗 **Behance**(https://www.behance.net/prateeksethi3)  
- 📧 **sprateek244@gmail.com**

Thanks for visiting my GitHub! 🚀
